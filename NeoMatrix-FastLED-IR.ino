...
Dear LLM:
* Before generating the code block, list the exact functions and the specific logic you are about to change. After the code block, perform a strict self-audit to explicitly confirm that the generated code contains the changes you promised
* Before finalizing this answer, act as a hostile code reviewer. Critically analyze the patch you just wrote for missing implementations, state-machine lockups, and unhandled variables. Tell me what you found and fix it.
* Do a step-by-step dry run of the network failure state through your new code to prove it behaves as claimed.
*/
...
#include "GifAnimViewer.h"
    ...
    // UDP portable discovery of slave to find ESP32 master (which in turn sends it back change pattern commands)
    #include <NetworkUdp.h>
    const uint16_t UDP_DISCOVERY_PORT = 4210;
    NetworkUDP udp; // Core 3.x native class type (WiFiUDP is just a typedef of this)
    uint32_t masterBootToken = 0;       // Generated by Master at boot
    IPAddress Master_IP(0, 0, 0, 0); // Dynamically updated by Slave
    ...
    // this may stop udp packets from being processed, turn off when not debugging
    // #define WIFI_SNIFFER
    #ifdef WIFI_SNIFFER
        #include <esp_wifi.h>
        // Volatile globals to pass data safely from ISR to the main thread
        volatile bool sniffer_caught_packet = false;
        volatile uint16_t sniffer_packet_len = 0;
        ...
        void IRAM_ATTR wifi_sniffer_cb(void *buf, wifi_promiscuous_pkt_type_t type) {
            ...
            // Brute-force search for Port 4210 (0x1072)
            for (uint16_t i = 0; i < len - 1; i++) {
                if (payload[i] == 0x10 && payload[i+1] == 0x72) {
                    // NEVER print here. Just capture the state and exit quickly.
                    sniffer_packet_len = len;
                    sniffer_caught_packet = true;
                    break;
                }
            }
        }
    ...
    volatile bool slave_ip_sent = false;

    void send_slave_ip_to_master() {
        if (send_slave_ip_to_master_tries > 15) return;
    ...
    void broadcast_master_presence() {
         if (!WIFI_SLAVE_MODE) {
             if (WiFi.status() == WL_CONNECTED || WiFi.getMode() == WIFI_AP || WiFi.getMode() == WIFI_AP_STA) {
                 ...
                 IPAddress broadcastIP = isAP ? IPAddress(255, 255, 255, 255) : WiFi.broadcastIP();
    ...
    void listen_for_master_broadcast() {
        if (WIFI_SLAVE_MODE) {
            ...
            #ifdef WIFI_SNIFFER
                if (sniffer_caught_packet) {
                    Serial.printf("\r\n[TCPDUMP] HARDWARE MAC RX: Port 4210 frame hit the antenna! (Len: %d)\r\n", sniffer_packet_len);
                    sniffer_caught_packet = false;
                }
            #endif
            ...
            if (WiFi.status() == WL_CONNECTED) {
                ...
                if (packetSize >= sizeof(uint32_t)) {
                    ...
                    udp.read((uint8_t*)&receivedToken, sizeof(receivedToken));
    ...
    // We only define splitter here because they include networking calls, and we 
    // want the above networking calls to use the native Serial.print without networking added.
...
/* 260 */   { ROOT  "F_Anna.gif"              , 10, 6,  9, 10, 10,140,210 },
...
/* 268 */   { ROOT  "DJ_Superstrings.gif"     , 10, 6,  9, 10, 10,148,210 },
...
/* 278 */   { ROOT  "DJ_7lions.gif",	        10, 6,  9, 10, 10,140,210 },
/* 279 */   { ROOT  "DJ_Above_and_Beyond3.gif",	10, 6,  9, 10, 10,140,210 },
/* 280 */   { ROOT  "DJ_Afik_Cohen.gif",	10, 6,  9, 10, 10,140,210 },
/* 281 */   { ROOT  "DJ_Alessandra_Roncone.gif",10, 6,  9, 10, 10,140,210 },
/* 282 */   { ROOT  "DJ_Alex_MORPH.gif",	10, 6,  9, 10, 10,140,210 },
/* 283 */   { ROOT  "DJ_Allen_Watts.gif",	10, 6,  9, 10, 10,140,210 },
/* 284 */   { ROOT  "DJ_Alpha9.gif",            10, 6,  9, 10, 10,140,210 },
/* 285 */   { ROOT  "DJ_Alvin.gif",		10, 6,  9, 10, 10,140,210 },
/* 286 */   { ROOT  "DJ_AlynFila_Fadi_.gif",	10, 0,  0, 10, 10,140,210 } ,
/* 287 */   { ROOT  "DJ_AlynFila_lgrey.gif",	10, 6,  9, 10, 10,100,192 },
/* 288 */   { ROOT  "DJ_Aname.gif",             10, 6,  9, 10, 10,140,210 },
/* 289 */   { ROOT  "DJ_AndrewRayel.gif",	10, 6,  9, 10, 10,140,210 },
/* 290 */   { ROOT  "DJ_Andy_Moor.gif",		10, 6,  9, 10, 10,140,210 },
/* 291 */   { ROOT  "DJ_Astrix.gif",		10, 6,  9, 10, 10,140,210 },
/* 292 */   { ROOT  "DJ_ATB.gif",		10, 6,  9, 10, 10,140,210 },
/* 293 */   { ROOT  "DJ_AvB.gif",	    	10, 6,  9, 10, 10,140,210 },
/* 294 */   { ROOT  "DJ_Ben_Gold.gif",		10, 6,  9, 10, 10,140,210 },
/* 295 */   { ROOT  "DJ_Ben_Nicky.gif",		10, 6,  9, 10, 10,140,210 },
/* 296 */   { ROOT  "DJ_Blastoyz.gif",		10, 6,  9, 10, 10,140,210 },
/* 297 */   { ROOT  "DJ_Blur.gif",              10, 6,  9, 10, 10,140,210 },
/* 298 */   { ROOT  "DJ_BryanKearney.gif",      10, 6,  9, 10, 10,140,210 },
/* 299 */   { ROOT  "DJ_Ciaran_McCauley.gif",	10, 6,  9, 10, 10,140,210 },
/* 300 */   { ROOT  "DJ_Cold_Blue.gif",		10, 6,  9, 10, 10,140,210 },
/* 301 */   { ROOT  "DJ_CosmicGate.gif",	10, 6,  9, 10, 10,140,210 },
/* 302 */   { ROOT  "DJ_Craig_Connelly.gif",	10, 6,  9, 10, 10,140,210 },
/* 303 */   { ROOT  "DJ_Crystal_Method.gif",	10, 6,  9, 10, 10,140,210 },
...
/* 316 */   { ROOT  "DJ_Ilan_Bluestone.gif",	10, 6,  9, 10, 10,140,210 },
/* 317 */   { ROOT  "DJ_JES.gif",		10, 6,  9, 10, 10,140,210 },
/* 318 */   { ROOT  "DJ_JOC.gif",		10, 6,  9, 10, 10,140,210 },
/* 319 */   { ROOT  "DJ_Johan_Gielen.gif",	10, 6,  9, 10, 10,140,210 },
/* 320 */   { ROOT  "DJ_John_00_Flemming.gif",	10, 6,  9, 10, 10,140,210 },
/* 321 */   { ROOT  "DJ_Judge_Jules.gif",		10, 6,  9, 10, 10,140,210 },
/* 322 */   { ROOT  "DJ_Khromata.gif",		10, 6,  9, 10, 10,140,210 },
/* 323 */   { ROOT  "DJ_Lange.gif",             10, 6,  9, 10, 10,140,210 },
/* 324 */   { ROOT  "DJ_Liquid_Soul.gif",	10, 6,  9, 10, 10,140,210 },
/* 325 */   { ROOT  "DJ_Mark_Sixma.gif",	10, 6,  9, 10, 10,140,210 },
/* 326 */   { ROOT  "DJ_Markus_Schulz.gif",     10, 6,  9, 10, 10,140,210 },
/* 327 */   { ROOT  "DJ_Marlo.gif",		10, 6,  9, 10, 10,140,210 },
/* 328 */   { ROOT  "DJ_MIKE_Push.gif",		10, 6,  9, 10, 10,140,210 },
...
/* 336 */   { ROOT  "DJ_PaulVanDyk.gif",	10, 6,  9, 10, 10,140,210 },
/* 337 */   { ROOT  "DJ_RAM.gif",               10, 6,  9, 10, 10,140,210 },
/* 338 */   { ROOT  "DJ_RDR_lgrey.gif",		10, 0,  0, 10, 10,128,128 },
/* 339 */   { ROOT  "DJ_Reality_Test.gif",	10, 6,  9, 10, 10,140,210 },
/* 340 */   { ROOT  "DJ_Richard_Durand.gif",    10, 6,  9, 10, 10,140,210 },
/* 341 */   { ROOT  "DJ_Rinaly.gif",		10, 6,  9, 10, 10,140,210 },
/* 342 */   { ROOT  "DJ_Roger_Shah.gif",		10, 6,  9, 10, 10,140,210 },
/* 343 */   { ROOT  "DJ_Ruben.gif",		10, 0,  0, 10, 10,128,170 } ,
/* 344 */   { ROOT  "DJ_Sean_Tyas.gif",		10, 6,  9, 10, 10,140,210 },
/* 345 */   { ROOT  "DJ_Solarstone.gif",	10, 6,  9, 10, 10,140,210 },
/* 346 */   { ROOT  "DJ_Somna.gif",		10, 6,  9, 10, 10,140,210 },
/* 347 */   { ROOT  "DJ_Sony.gif",              10, 6,  9, 10, 10,140,210 },
/* 348 */   { ROOT  "DJ_Susana.gif",		10, 6,  9, 10, 10,140,210 },
/* 349 */   { ROOT  "DJ_Talla-2XLC.gif",	10, 6,  9, 10, 10,140,210 },
/* 350 */   { ROOT  "DJ_Thrillseekers.gif",	10, 6,  9, 10, 10,140,210 },
/* 351 */   { ROOT  "DJ_Triode.gif",	        10, 6,  9, 10, 10,140,210 },
/* 352 */   { ROOT  "DJ_Ulrich.gif",		10, 6,  9, 10, 10,140,210 },
/* 353 */   { ROOT  "DJ_Vini_Vici.gif",		10, 0 , 0, 10, 10,128,140 },
/* 354 */   { ROOT  "DJ_WhiteNo1se.gif",	10, 6,  9, 10, 10,140,210 },
/* 355 */   { ROOT  "DJ_Will_Atkinson.gif",	10, 6,  9, 10, 10,140,210 },
/* 356 */   { ROOT  "DJ_Xijaro_Anthony.gif",		10, 6,  9, 10, 10,140,210 },
/* 357 */   { ROOT  "DJ_Xijaro_n_Pitch.gif",		10, 6,  9, 10, 10,140,210 },
...
/* 263 */ { "GIF inca spiralin",	 GifAnim, 143, NULL },
...
// dupe all the lines, cut out just the GIF text, sort, and square paste back
/* 398 */ { "GIF 7lions",            GifAnim, 278, NULL },
/* 399 */ { "GIF Above and Beyond",  GifAnim, 279, NULL },
/* 400 */ { "GIF Afik Cohen",        GifAnim, 280, NULL },
/* 401 */ { "GIF Alessandra Roncone",GifAnim, 281, NULL },
/* 402 */ { "GIF Alex MORPH",        GifAnim, 282, NULL },
/* 403 */ { "GIF Allen Watts",       GifAnim, 283, NULL },
/* 404 */ { "GIF Alpha9",            GifAnim, 284, NULL },
/* 405 */ { "GIF Alvin",             GifAnim, 285, NULL },
/* 406 */ { "GIF Aly & Fila Fadi",   GifAnim, 286, NULL },
/* 407 */ { "GIF Aly & Fila Logo",   GifAnim, 287, NULL },
/* 408 */ { "GIF Aname",             GifAnim, 288, NULL },
/* 409 */ { "GIF Andrew Rayel",      GifAnim, 289, NULL },
/* 410 */ { "GIF Andy Moor",         GifAnim, 290, NULL }, // deno 330
/* 411 */ { "GIF Astrix",            GifAnim, 291, NULL },
/* 412 */ { "GIF ATB",               GifAnim, 292, NULL },
/* 413 */ { "GIF AvB",               GifAnim, 293, NULL },
/* 414 */ { "GIF Ben Gold",          GifAnim, 294, NULL },
/* 415 */ { "GIF Ben Nicky",         GifAnim, 295, NULL },
/* 416 */ { "GIF Blastoyz",          GifAnim, 296, NULL },
/* 417 */ { "GIF Blur",              GifAnim, 297, NULL },
/* 418 */ { "GIF Bryan Kearney",     GifAnim, 298, NULL },
/* 419 */ { "GIF Ciaran McCauley",   GifAnim, 299, NULL },
/* 420 */ { "GIF Cold Blue",         GifAnim, 300, NULL }, // demo 340
/* 421 */ { "GIF Cosmic Gate",       GifAnim, 301, NULL },
/* 422 */ { "GIF Craig Connelly",    GifAnim, 302, NULL },
/* 423 */ { "GIF Crystal Method",    GifAnim, 303, NULL },
/* 424 */ { "GIF Dave Dresden",      GifAnim, 304, NULL },
/* 425 */ { "GIF Daxson",            GifAnim, 305, NULL },
/* 426 */ { "GIF Emma Hewitt",       GifAnim, 306, NULL },
/* 427 */ { "GIF Eric Prydz",        GifAnim, 307, NULL },
/* 428 */ { "GIF FactorB",           GifAnim, 308, NULL },
/* 429 */ { "GIF Ferry Corsten",     GifAnim, 309, NULL },
/* 430 */ { "GIF Ferry Tayle",       GifAnim, 310, NULL }, // demo 350
/* 431 */ { "GIF Gareth Emery",      GifAnim, 311, NULL },
/* 432 */ { "GIF Genix",             GifAnim, 312, NULL },
/* 433 */ { "GIF Giuseppe",          GifAnim, 313, NULL },
/* 434 */ { "GIF Giuseppe Logo",     GifAnim, 314, NULL },
/* 435 */ { "GIF Haliene",           GifAnim, 315, NULL },
/* 436 */ { "GIF Ilan Bluestone",    GifAnim, 316, NULL },
/* 437 */ { "GIF JES",               GifAnim, 317, NULL },
/* 438 */ { "GIF JOC",               GifAnim, 318, NULL },
/* 439 */ { "GIF Johan Gielen",      GifAnim, 319, NULL },
/* 440 */ { "GIF John 00 Flemming",  GifAnim, 320, NULL }, // demo 360
/* 441 */ { "GIF Judge Jules",       GifAnim, 321, NULL },
/* 442 */ { "GIF Khromata",          GifAnim, 322, NULL },
/* 443 */ { "GIF Lange",             GifAnim, 323, NULL },
/* 444 */ { "GIF Liquid Soul",       GifAnim, 324, NULL },
/* 445 */ { "GIF Mark Sixma",        GifAnim, 325, NULL },
/* 446 */ { "GIF Markus Schulz",     GifAnim, 326, NULL },
/* 447 */ { "GIF Marlo",             GifAnim, 327, NULL },
/* 448 */ { "GIF MIKE Push",         GifAnim, 328, NULL },
/* 449 */ { "GIF Miyuki",            GifAnim, 329, NULL },
/* 450 */ { "GIF Monique",           GifAnim, 330, NULL }, // demo 370
/* 451 */ { "GIF Nifra",             GifAnim, 331, NULL },
/* 452 */ { "GIF Nilsix",            GifAnim, 332, NULL },
/* 453 */ { "GIF Orjan Nilsen",      GifAnim, 333, NULL },
/* 454 */ { "GIF Orkidea",           GifAnim, 334, NULL },
/* 455 */ { "GIF Paul Oakenfold",    GifAnim, 335, NULL },
/* 456 */ { "GIF Paul Van Dyk",      GifAnim, 336, NULL },
/* 457 */ { "GIF RAM",               GifAnim, 337, NULL },
/* 458 */ { "GIF RDR Logo",          GifAnim, 338, NULL },
/* 459 */ { "GIF Reality Test",      GifAnim, 339, NULL },
/* 460 */ { "GIF Richard Durand",    GifAnim, 340, NULL }, // demo 380
/* 461 */ { "GIF Rinaly",            GifAnim, 341, NULL },
/* 462 */ { "GIF Roger_Shah",        GifAnim, 342, NULL },
/* 463 */ { "GIF RubenDeRonde",      GifAnim, 343, NULL },
/* 464 */ { "GIF Sean Tyas",         GifAnim, 344, NULL },
/* 465 */ { "GIF Solarstone",        GifAnim, 345, NULL },
/* 466 */ { "GIF Somna",             GifAnim, 346, NULL },
/* 467 */ { "GIF Sony",              GifAnim, 347, NULL },
/* 468 */ { "GIF Susana",            GifAnim, 348, NULL },
/* 469 */ { "GIF Talla-2XLC",        GifAnim, 349, NULL },
/* 470 */ { "GIF Thrillseekers",     GifAnim, 350, NULL }, // demo 390
/* 471 */ { "GIF Triode",            GifAnim, 351, NULL },
/* 472 */ { "GIF Ulrich",	           GifAnim, 352, NULL },
/* 473 */ { "GIF Vini Vici",         GifAnim, 353, NULL },
/* 474 */ { "GIF WhiteNo1se",        GifAnim, 354, NULL },
/* 475 */ { "GIF Will Atkinson",     GifAnim, 355, NULL },
/* 476 */ { "GIF Xijaro Anthony",    GifAnim, 356, NULL },
/* 477 */ { "GIF Xijaro n Pitch",    GifAnim, 357, NULL },
/* 478 */ { "GIF Yoji Biomehanika",  GifAnim, 358, NULL }, // demo 398
...
void matrix_change(int16_t demo, bool directmap=false, int16_t loop=-1) {
    ...
    MS_Print(); Serial.print("got matrix_change code ");
    Serial.print(demo);
...
void Matrix_Handler() {
	...
	if (cnt++ % 10 == 0) {
#ifdef ESP32
    // in dumb framebuffer mode with no output, FPS does not matter
    #ifndef FRAMEBUFFER
    ...
    MATRIX_RESET_DEMO = true;
    MS_Print(); Serial.print("Done with demo ");
    Serial.print(MATRIX_DEMO);
    Serial.print(" loop ");
    Serial.println(MATRIX_LOOP);
...
void change_brightness(int8_t change, bool absolute=false) {
    ...
    // rgbpanels are dim, bump up brightness
        ...
        uint8_t rgbpanel_brightness = 31+min( (1 << (brightness)), 224);
    #else
		...
		if (Serial.available()) readchar = Serial.read();
            ...
            if (new_pattern > 16777216) {
                ...
                if (new_pattern >= 0x100000000ULL) {
                    ...
                    Serial.println(RPI_IP.toString().c_str());
                }
            }
            #endif
...
void connectionStatus(const char *ssid, bool trying, bool failure, bool success)
{
  ...
  if (trying)       { 
      what = "trying"; 
      wifi_network_stable = false;
      Splitter.restart(); // CRITICAL: Stop UDP poll on interface drop
  }
  else if (failure) { 
      what = "failure"; 
      failure_cnt++; 
      ...
      failure_cnt = 0; 
      send_slave_ip_to_master_tries = 0;
      slave_ip_sent = false;      // Forces execution update on local drop
...
void setup_wifi() {
    ...
    if (WIFI_SLAVE_MODE) {
        ...
        // the next step
        Serial.println(">>>>>>>>>>>>>> Wifi Configured for Wifi AP mode, will try to connect to " WIFI_SSID);
        WebServer->addWifi(WIFI_SSID, WIFI_PASSWORD);
    }
...
void loop() {
	...
	EVERY_N_SECONDS(60) {
          MS_Print(); Serial.printf("Loop() - Free Stack Space: %d", uxTaskGetStackHighWaterMark(NULL));
          show_free_mem("");
	}
    #ifdef WIFI
        ArduinoOTA.handle();
        Splitter.handle(); 
        listen_for_master_broadcast(); // Slave reads this continuously
        EVERY_N_SECONDS(5) {
            broadcast_master_presence(); // Master broadcasts presence
